var textureFlowOG = null;
function initTextureFlowOG(event){
    event.preventDefault();
    if(textureFlowOG == null){
        document.getElementById("textureFlowOG").children[0].style.display = "none";       
        textureFlowOG = new TextureFlowOG();
        textureFlowOG.init();
        tfOGAnimate();
    } else {
        return;
...
function tfOGAnimate(){
    window.requestAnimationFrame(tfOGAnimate);
    textureFlowOG.draw();
}

function TextureFlowOG(){
    ...
    this.init = function(){
        ...
        this.container = document.getElementById( 'textureFlowOG' );
        this.container.appendChild( this.renderer.domElement );

        var inputTexture = THREE.ImageUtils.loadTexture("tex/tex03.jpg");
        ...
        this.rtt1.minFilter = this.rtt2.minFilter = THREE.LinearFilter;
        this.rtt1.magFilter = this.rtt2.magFilter = THREE.LinearFilter;
        ...
        this.mat2 = new THREE.ShaderMaterial({
            uniforms: textureFlowOGShader.uniforms,
            vertexShader: textureFlowOGShader.vertexShader,
            fragmentShader: textureFlowOGShader.fragmentShader
        });
        this.mat2.uniforms["texture"].value = this.rtt1;
        this.mat2.uniforms["time"].value = this.time;
