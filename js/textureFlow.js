var textureFlow = null;
function initTextureFlow(event){
    event.preventDefault();
    if(textureFlow == null){
        document.getElementById("textureFlow").children[0].style.display = "none";       
        textureFlow = new TextureFlow();
        textureFlow.init();
...
function tfAnimate(){
    window.requestAnimationFrame(tfAnimate);
    textureFlow.draw();
}

function TextureFlow(){
    ...
    this.init = function(){
        ...
        this.container = document.getElementById( 'textureFlow' );
